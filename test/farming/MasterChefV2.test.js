...
describe('Contract farming/MasterChefV2.sol', function () {
    const [
        owner,
        admin,
        manager,
    ...
    it('should deposit/withdraw lp and rewards', async function () {
        ...
        await expectRevert(
             this.masterChef.deposit(
                ...
                ether('1000'),
                { from: user }),
            'MasterChefV2: WL permission denied');
    ...
    describe('update sushiPerBlock logic', function () {
        it('should call setSushiPerBlock by owner', async function () {
            await this.masterChef.setSushiPerBlock(ether('0'), true, { from: owner });
            (await this.masterChef.sushiPerBlock()).should.be.zero;
        });

        it('should fail on call setSushiPerBlock by anyone', async function () {
            await expectRevert(
                this.masterChef.setSushiPerBlock(ether('0'), true, { from: anyone }),
                'Ownable: caller is not the owner'
            );
        });

        it('should deposit/withdraw lp and rewards after update sushiPerBlock', async function () {
            await this.masterChef.setSushiPerBlock(ether('2'), true, { from: owner });
            ...
            (await this.tokenRewards.balanceOf(user))
                .should.be.bignumber.equal(ether('8'));
    
            (await this.tokenRewards.balanceOf(this.reservoir.address))
                .should.be.bignumber.equal(INITIAL_SUPPLY.sub(ether('8')));
        });
    });
});
